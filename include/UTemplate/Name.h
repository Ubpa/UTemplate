...
// Checks UBPA_NAME_enum compiler compatibility.
#if defined(__clang__) && __clang_major__ >= 5 || defined(__GNUC__) && __GNUC__ >= 9 || defined(_MSC_VER) && _MSC_VER >= 1910
#  undef  UBPA_NAME_ENUM_SUPPORTED
#  define UBPA_NAME_ENUM_SUPPORTED 1
#endif
...
namespace Ubpa {
	...
	constexpr auto constexpr_name() {
		...
		if constexpr (std::is_null_pointer_v<T>)
			...
			if constexpr (V == nullptr)
				...
				return concat_seq(TSTR("&"), type_name<FuncTraits_Obj<T>>(), TSTR("::"), details::member_pointer_name_v<V>);
		}
	...
	constexpr auto type_name() noexcept {
		if constexpr (std::is_lvalue_reference_v<T>)
			...
			constexpr auto RetName = concat_seq(TSTR("{"), type_name<Ret>(), TSTR("}"));
